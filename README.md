# Decision-making-statements

Aim: To study and implement C++ decision making statements.

Theory:Decision-making statements in C++ are essential for controlling the flow of a program based on certain conditions. These statements evaluate conditions and execute specific code blocks depending on whether the conditions are true or false. The primary decision-making statements in C++ include if, else if, else, switch, and conditional (ternary) operators. The if statement allows for executing a block of code if a condition is true, and can be extended with else if and else for multiple conditions. The switch statement provides a way to execute different code blocks based on the value of a variable, making it a suitable choice for situations with multiple possible discrete values. The conditional operator ?: offers a compact syntax for simple if-else conditions. These decision-making constructs enable developers to create dynamic and responsive programs that can adapt to varying inputs and scenarios, forming the backbone of logic control in software applications
## algorithm
1. to make a simple calculator using switch case
   
Start

Display the menu of operations.

Input the choice of operation (op), and two numbers (num1 and num2).

Process:
If op is '1', Output the sum of num1 and num2.

If op is '2', Output the difference between num1 and num2.

If op is '3', Output the product of num1 and num2.

If op is '4':

If num2 is 0, Output "Error! Division by zero."

Else, Output the quotient of num1 divided by num2.

Else, Output "Error! Invalid operator."

End

2 to print weekdays using switch case

Start

Display the list of days of the week with corresponding numbers (1 to 7).

Input the user's choice (choice).

Process:

If choice is 1, Output "You selected Monday."

If choice is 2, Output "You selected Tuesday."

If choice is 3, Output "You selected Wednesday."

If choice is 4, Output "You selected Thursday."

If choice is 5, Output "You selected Friday."

If choice is 6, Output "You selected Saturday."

If choice is 7, Output "You selected Sunday."

Else, Output "Invalid choice. Please enter a number between 1 and 7."

End
3 quadrant checker

Start

Input: Read the values of x and y.

Process:

If x > 0 and y > 0, Output "The point is in the first quadrant."

Else If x < 0 and y > 0, Output "The point is in the second quadrant."

Else If x < 0 and y < 0, Output "The point is in the third quadrant."

Else If x > 0 and y < 0, Output "The point is in the fourth quadrant."

Else If x == 0 or y == 0, Output "The point is on the axis."

Else, Output "The point is on the origin."
